...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      chars.groupBy(x=>x) map {case (a,b) => (a, b.size)} toList
    }
   ...
   */
    ...
    def ins(x: (Char, Int), xs: List[(Char, Int)]): List[(Char, Int)] = xs match {
      case List() => List()
      case y::ys => if (y._2>x._2) x::xs else y::ins(x, ys)
    }
   ...
   */
    ...
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      ...
      trees match {
        case Nil => Nil
        case x::Nil => x::Nil
        case x::y::z => (makeCodeTree(x, y) :: z).sortBy(weight(_))
      }
    }
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
      val treeList = makeOrderedLeafList(times(chars))
      until(singleton, combine)(treeList)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def func(tree: CodeTree)(c: Char): List[Bit] = tree match {

        case Leaf(_, _) => List()

        case Fork(l, r, _, _) => l match {
            case x if chars(x).contains(c) => 0::func(l)(c)
            case _ => 1::func(r)(c)
        }
      }
      text.flatMap(func(tree))
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = {
      table.filter(x=>x._1==char).map(x=>x._2).head
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      ...
      case Fork(l,r,c,_) => mergeCodeTables(convert(r), convert(r))
   ...
   */
    def mergeCodeTables(l: CodeTable, r: CodeTable): CodeTable = {
      def prep(t: Bit)(c: (Char, List[Bit])): (Char, List[Bit]) = (c._1, t::c._2)

      l.map(prep(0)) ::: r.map(prep(1))
    }
